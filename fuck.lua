...
local function waitForTouch()
  while true do
    ...
    elseif side == peripheral.getName(monitor) then
      -- Only allow tap if current player taps
...
local function drawBettingButtons(mode)
  ...
  if mode == "game" then
    ...
    bidMonitor.setTextColor(colors.black)
    bidMonitor.setBackgroundColor(colors.orange)
    bidMonitor.setCursorPos(2, 3) bidMonitor.write("<  +500   >")
    bidMonitor.setCursorPos(2, 4) bidMonitor.write("<  +1000  >")
    bidMonitor.setCursorPos(2, 5) bidMonitor.write("< +10000  >")
    bidMonitor.setCursorPos(2, 6) bidMonitor.write("<  -500   >")
    bidMonitor.setCursorPos(2, 7) bidMonitor.write("< -1000   >")
    bidMonitor.setCursorPos(2, 8) bidMonitor.write("< -10000  >")
    bidMonitor.setCursorPos(2, 10) bidMonitor.write("[ MAX BET ]")
  end
end
...
local function playBlackjack()
  ...
  -- Draw final hands with dealer revealed
  drawGame(playerHand, dealerHand, false)
  ...
  if didPlayerWin(playerVal, dealerVal) then
    ...
    if isBlackjack(playerHand) then
      winnings = math.ceil(currentBet * 2)
      updateBalance(currentPlayer, winnings, "blackjack natural win")
      resultText = "Blackjack! You Win! +" .. winnings .. " coins"
    else
      winnings = math.ceil(currentBet * BLACKJACK_MULTIPLIER)
  ...
  -- Show result below cards on the same screen
  monitor.setTextColor(colors.white)
  local w, h = monitor.getSize()
  local resultLine = h - 2
  monitor.setCursorPos(1, resultLine)
  monitor.setBackgroundColor(colors.blue)
  monitor.clearLine()
  monitor.write(string.rep(" ", w)) -- clear entire line for result
  monitor.setCursorPos(1, resultLine)
  monitor.write("Result: " .. resultText)
  
  monitor.setCursorPos(1, h)
  monitor.write("Tap screen to continue...")

  while true do
    if waitForTouch() == "ack" then break end
  end
