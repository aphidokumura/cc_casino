...
local function getLoggedInPlayer()
  ...
  for _, p in ipairs(players) do
    if p.distance <= MAX_DISTANCE and p.distance < closestDist then
...
local function didPlayerWin(playerVal, dealerVal)
  ...
  elseif playerVal == dealerVal then return math.random() < 0.40
...
-- === Debug Print to console/log (You can see this in CC terminal) ===
local function debugPrint(msg)
  print("[DEBUG] " .. msg)
end

-- === The complete playBlackjack function with debug ===
local function playBlackjack()
  local playerHand = { drawCard(), drawCard() }
  local dealerHand = { drawCard(), drawCard() }

  debugPrint("Game started for player: " .. currentPlayer)
  debugPrint("Initial player hand: " .. table.concat(playerHand, ", "))
  debugPrint("Initial dealer hand: " .. table.concat(dealerHand, ", "))

  -- Player turn
  while handValue(playerHand) < 21 do
    ...
    if action == "hit" then
      ...
      debugPrint("Player hits and draws: " .. newCard)
      debugPrint("Player hand now: " .. table.concat(playerHand, ", "))

      -- Redraw IMMEDIATELY with updated hand
      drawGame(playerHand, dealerHand, true)
      drawBettingButtons("game")
      drawActionButtons()

      sleep(0.5) -- Small delay so redraw is visible
    else
      debugPrint("Player stands")
      break
    end
  end
  ...
  while handValue(dealerHand) < 17 do
    ...
    debugPrint("Dealer hits and draws: " .. newCard)
  ...
  debugPrint("Game ended for player: " .. currentPlayer .. " with result: " .. resultText)
  sleep(WIN_DELAY + 1)
end
