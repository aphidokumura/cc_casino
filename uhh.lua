...
local function playBlackjack()
  ...
  -- Display result text below cards
  local _, screenHeight = monitor.getSize()
  monitor.setCursorPos(2, screenHeight - 3)
  monitor.setTextColor(colors.white)
  monitor.setBackgroundColor(colors.blue)
  monitor.write("Result: " .. resultText)

  monitor.setCursorPos(2, screenHeight - 2)
  monitor.write("Touch to return to menu...")

  while true do
    local event, side = os.pullEvent("monitor_touch")
    if side == peripheral.getName(bidMonitor) or side == peripheral.getName(actionMonitor) then
      break
    end
  end
