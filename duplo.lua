-- Double Up (Final Rednet-Based, No Firebase, Proper Winnings Update + UI Reset Fix)
...
local function showBettingUI(player, balance, bet)
  ...
  bid.clear()
  center(bid, 1, "Balance: " .. balance, colors.gray, colors.lime)
  center(bid, 2, "Bet: " .. bet, colors.black, colors.lime)
  center(bid, 3, "[<]Lower Raise[>]", colors.purple, colors.pink)
  center(bid, 4, "[MAX: 500]", colors.purple, colors.pink)
end
  ...
  while true do
    ...
    if side == peripheral.getName(bid) then
      if y == 3 then
        ...
        startTime = os.clock()
      elseif y == 4 and x >= 2 and x <= 20 then
        index = #betOptions
        bet = betOptions[index]
        startTime = os.clock()
      end
    elseif side == peripheral.getName(act) then
      if y == 2 and x >= 2 and x <= 20 then
        if bet > balance then
          main.clear()
...
while true do
  ...
  while not lockedPlayer do
    ...
    if not name then
      ...
      bid.setTextColor(colors.purple)
      bid.clear()
      center(bid, 1, "Balance: 0", colors.gray, colors.lime)
      center(bid, 2, "Bet: 0", colors.black, colors.lime)
      center(bid, 3, "[<]Lower Raise[>]", colors.purple, colors.pink)
      center(bid, 4, "[MAX: 500]", colors.purple, colors.pink)
